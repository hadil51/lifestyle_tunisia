...
const SECTION_KEYS = ["latest", "formal", "casual", "street"];
const PER_SECTION = 15;

function assignProductSections(products) {
  return products.map((product, index) => ({
    ...product,
    section: SECTION_KEYS[Math.min(Math.floor(index / PER_SECTION), SECTION_KEYS.length - 1)],
    rating: product.rating ?? 5,
  }));
}

export function mapUserContentToSite(sourceData) {
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  const mapped = mapUserContentToSite(source);
